...
export class VersionSelector extends HTMLElement {
  ...
  async fetchVersionJson() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        branches.map((branch) => this.DOC_VERSIONS.push(branch)).sort()
      }
    }
    catch (err) {
      console.warn(err)
    ...
    this.DOC_VERSION_LATEST = isLocalDev ? 'Local' : branches[0]; // assign local
  }
  ...
  async connectedCallback() {
    ...
    frag
      ...
      .appendChild(this._makeFragment(versionsDOMText));
    frag
  ...
  _instrument(shadowRoot) {
    ...
    shadowRoot.querySelector('#dropdown').addEventListener('pointerup', (e) => {
      const { target } = e;
      if (target.matches('a[href]') && target.href)
