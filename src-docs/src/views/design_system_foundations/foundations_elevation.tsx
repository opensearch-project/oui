...
const lightElevationLevels: ElevationLevel[] = [
  {
    level: 0,
    variable: 'none',
    description: 'Flat — no elevation. Used for inline content and flush surfaces.',
    cssValue: 'none',
  },
  {
    level: 1,
    ...
    <OuiFlexItem style={{ minWidth: 280, maxWidth: 400 }}>
      <div
        style={{
          ...
          boxShadow: elevation.cssValue === 'none' ? 'none' : elevation.cssValue,
          padding: 24,
          minHeight: 120,
          display: 'flex',
...
const FoundationsElevation: React.FC<ContentPageProps> = ({ selectedTheme }) => {
    ...
    <GuidePage title="Elevation">
      <OuiText grow={false}>
        ...
        <p>
          ...
          users understand which elements sit above others, guiding focus
          toward floating surfaces like popovers, modals, and notifications.
        </p>
      </OuiText>
      ...
      <OuiText size="s">
        <p>
          ...
          <OuiCode transparentBackground>ouiElevation()</OuiCode> mixin,
          which maps a numeric level to the corresponding shadow token:
        </p>
      </OuiText>
      <OuiSpacer size="m" />
...
.my-modal {
  @include ouiElevation(5); // High-priority overlay
      ...
      <OuiText size="s">
        <p>
          For components that create a local stacking context, combine
          elevation with the{' '}
          <OuiCode transparentBackground>ouiLocalElevation</OuiCode> mixin:
        </p>
      </OuiText>
      <OuiSpacer size="m" />
      ...
      <OuiText size="s">
        <ul>
          <li>
            ...
            between states. For example, a card at level 1 should rise to
            level 2 on hover, not jump to level 4.
          </li>
          <li>
            <strong>Use local stacking contexts</strong> (
            <OuiCode transparentBackground>ouiLocalElevation</OuiCode>) to
            prevent z-index conflicts between independent component trees.
            This uses <OuiCode transparentBackground>isolation: isolate</OuiCode>{' '}
            to scope z-index values.
          </li>
          <li>
            <strong>Pair elevation with background color</strong> changes in
            dark mode. Elevated surfaces should use a slightly lighter
            background to reinforce the sense of depth, since shadows are
            less visible against dark backgrounds.
          </li>
          <li>
            <strong>Keep the page surface flat</strong> (level 0). Only
            elements that need to communicate layering — cards, popovers,
            modals — should receive elevation.
          </li>
          <li>
            <strong>Avoid stacking elevated elements</strong> unnecessarily.
            If a popover opens from within a modal, the popover should use
            level 3 relative to its local context, not level 6.
          </li>
        </ul>
      </OuiText>
          ...
          <OuiCallOut
            ...
            <OuiText size="s">
              <p>
                ...
                shadow layer increases in offset and blur from level 1 to 6.
                In dark mode, the base layer switches to a light-ring glow
                using zinc-400 tones so that elevated surfaces remain
                perceptible against dark backgrounds. Shadow opacity increases
                from 0.32 (light) to 0.90 (dark) to compensate for reduced
                contrast.
              </p>
            </OuiText>
          </OuiCallOut>
